# Copyright (C) 2022-2026 by the FEM on Kaggle authors
#
...
remove_kaggle_packages () {
    grep -v -e "^a2a-sdk" -e "^alembic" -e "^annoy" -e "^astropy" -e "^bayesian-optimization" -e "^Boruta" -e "bq_helper$" -e "^Cartopy" -e "^category-encoders" -e "^category_encoders" -e "^cesium" -e "^cloud-sql-python-connector" -e "^clu" -e "^datashader" -e "^deap" -e "^dipy" -e "^docker" -e "^easyocr" -e "^eli5" -e "^fasttext" -e "^featuretools" -e "^fury" -e "^gemma" -e "^h2o" -e "^hep-ml" -e "^hep_ml" -e "^holoviews" -e "^imutils" -e "^in-toto" -e "^ipython-sql" -e "^jieba" -e "^jupytext" -e "^kauldron" -e "^kornia" -e "^kubernetes" -e "^learntools" -e "^lime" -e "^litellm" -e "^mdit-py-plugins" -e "^mlcrate" -e "^moviepy" -e "^nilearn" -e "^nltk" -e "^onnx" -e "^optuna" -e "^pandas-profiling" -e "^pandasql" -e "^panel" -e "^plotnine" -e "^preprocessing" -e "^pyLDAvis" -e "^pymc3" -e "^ray" -e "^rgf-python" -e "^rouge_score" -e "^s3fs" -e "^segment_anything" -e "^sigstore" -e "^SQLAlchemy" -e "^stable-baselines" -e "^textblob" -e "^Theano" -e "^TPOT" -e "^vtk" -e "^xmanager" -e "^ydata-profiling" -e "^wandb" -h ${1} > ${1}.tmp
    mv ${1}.tmp ${1}
}
