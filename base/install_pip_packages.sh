...
remove_kaggle_packages () {
    grep -v -e "^annoy" -e "^bayesian-optimization" -e "betterproto" -e "^Boruta" -e "bq_helper$" -e "^Cartopy" -e "^category-encoders" -e "^category_encoders" -e "^cesium" -e "^cuml" -e "^cuvs" -e "^dask" -e "^datasets" -e "^datashader" -e "^deap" -e "^dipy" -e "^distributed" -e "^docker" -e "^easyocr" -e "^eli5" -e "^fasttext" -e "^featuretools" -e "^h2o" -e "^hep-ml" -e "^hep_ml" -e "^imutils" -e "^jieba" -e "^kaggle" -e "^kornia" -e "^learntools" -e "^libpysal" -e "^lime" -e "^mlcrate" -e "^model-signing" -e "^nilearn" -e "^nltk" -e "^optuna" -e "^plotnine" -e "^preprocessing" -e "^pyLDAvis" -e "^pymc3" -e "^pytorch" -e "^raft-dask" -e "^rapids-dask" -e "^rgf-python" -e "^segment_anything" -e "^sigstore" -e "^stable-baselines" -e "^textblob" -e "^Theano" -e "^TPOT" -e "^tsfresh" -e "^ucxx" -e "^wandb" -h ${1} > ${1}.tmp
    mv ${1}.tmp ${1}
}
