...
remove_kaggle_packages () {
    grep -v -e "^annoy" -e "^astropy" -e "^bayesian-optimization" -e "^Boruta" -e "bq_helper$" -e "^Cartopy" -e "^category-encoders" -e "^category_encoders" -e "^cesium" -e "^datashader" -e "^deap" -e "^dipy" -e "^docker" -e "^easyocr" -e "^eli5" -e "^fasttext" -e "^featuretools" -e "^h2o" -e "^hep-ml" -e "^hep_ml" -e "^imutils" -e "^in-toto" -e "^jieba" -e "^kornia" -e "^learntools" -e "^lime" -e "^mlcrate" -e "^nilearn" -e "^nltk" -e "^onnx" -e "^opentelemetry" -e "^optuna" -e "^pandas-profiling" -e "^plotnine" -e "^preprocessing" -e "^pyLDAvis" -e "^pymc3" -e "^rgf-python" -e "^segment_anything" -e "^sigstore" -e "^stable-baselines" -e "^textblob" -e "^Theano" -e "^TPOT" -e "^wandb" -h ${1} > ${1}.tmp
    mv ${1}.tmp ${1}
}
